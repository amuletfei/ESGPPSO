...
class PSO():
    def __init__(self, n_div, le):
        self.myHPA101 = HPA101(n_div, le)
        self.Search_Agents = 50
        self.dimensions = self.myHPA101.nx
        ...
        self.model_path = f"./tra_data_HPA101/pso/models/cvae_pso_N{self.n_div}_L{self.level}.pth"
        self.checkpoint = None
        self.cvae_model = None
        if os.path.exists(self.model_path):
            self.checkpoint = torch.load(self.model_path, map_location=self.device, weights_only=False)

            cond_dim = self.Search_Agents * self.dimensions + self.Search_Agents
            self.cvae_model = CVAE(
                condition_dim=cond_dim,
                target_dim=cond_dim,
                latent_dim=self.checkpoint['latent_dim']
            ).to(self.device)
            self.cvae_model.load_state_dict(self.checkpoint['model_state_dict'])
    ...
    def predict_cvae(self, pop, fitness):
        if self.cvae_model is None or self.checkpoint is None:
            return None

        try:
            norm_pop = self._normalize_pop(pop, self.checkpoint['pop_min'], self.checkpoint['pop_max'])
            norm_fit = self._normalize_fit(fitness, self.checkpoint['log_fit_min'], self.checkpoint['log_fit_max'])
            ...
            with torch.no_grad():
                z = torch.randn(1, self.checkpoint['latent_dim']).to(self.device)
                recon_target = self.cvae_model.decode(z, condition_tensor)

            recon_pop_norm = recon_target[0, :self.Search_Agents * self.dimensions].cpu().numpy()
            cvae_pop_1d = self._denormalize_pop(recon_pop_norm, self.checkpoint['pop_min'], self.checkpoint['pop_max'])
    ...
    def PSO_searcher(self, run_index):
        np.random.seed(2026 + self.n_div + self.level + run_index)
                ...
                if cvae_pop is not None:
                    ...
                    for i, idx in enumerate(best_indices):
                        ...
                        if idx < self.Search_Agents:
                            ...
                            new_V[i] = V[v_ind]* np.random.uniform(-0.2, 0.2)
    ...
    def PSO_ReRun(self, Recount):
        ...
        plt.title(f'Convergence Curve of HPA {self.n_div}level{self.level}  (Log Scale)')
        plt.xlabel("# of fitness evaluations")
        plt.ylabel("Log Average Fitness Score")
        plt.grid(True, which="both", ls="--")

        dir_path_pic = f'./MCResult/HPA101/{self.n_div}N/{self.level}le/Pic/'
        os.makedirs(dir_path_pic, exist_ok=True)
        plt.savefig(dir_path_pic + f'mcPSOHPA101_N{self.n_div}le{self.level}.png')
        plt.close()
